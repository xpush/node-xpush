...
SessionManager.prototype.update = function(app, channel, server, count, callback) {
  ...
  if(callback){
    if (count > 0) {
      // TODO expirement time 추가 ( Server Config 에서 가져오 값으로. )
      this.redisClient.hset(app+":"+channel, server, count, callback);
    } else {
      this.redisClient.hdel(app+":"+channel, server, callback);
    }
  }else{
    if (count > 0) {
      // TODO expirement time 추가 ( Server Config 에서 가져오 값으로. )
      this.redisClient.hset(app+":"+channel, server, count);
    } else {
      this.redisClient.hdel(app+":"+channel, server);
    }
  }
};
