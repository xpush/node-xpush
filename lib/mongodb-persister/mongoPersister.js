...
exports.addActiveUser = function(input, done) { // A, C, U, DT
  var userObj={};
  userObj[input.U] = input.DT;
  var query= "{\"A\":\""+input.A+"\", \"C\":\""+input.C+"\", \"US\": {\"$elemMatch\": {\""+input.U+"\":{\"$exists\": true}}}  }";
	var data = {A:input.A, C:input.C};
	SessionUser.findOne(data,function(err,au){
		if(err) return done(err);

		if(au){
			SessionUser.findOne(JSON.parse(query),function(err,doc){
				if(err) return done(err);
				if(doc){

					var query = { A: input.A, C: input.C  };
				  var data= "{ \"$pull\": {\"US\": {\""+input.U+"\":{\"$exists\": true}}}}";
				  SessionUser.update(query, JSON.parse(data), { multi: true }, function(err) {
				    if (err) {
				      return done(err);
				    }else{
				    	doc.US.push(userObj);
...
exports.retrieveActiveUser = function(input, done) { // A, C
  ...
  SessionUser.findOne(query, {'_id': 0, US:1} ,function(err,doc){
	if(err) return done(err);
