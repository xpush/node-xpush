...
exports.addActiveUser = function(input, done) { // A, C, U, DT
	...
	var DT = input.DT;
	if( typeof input.DT == 'string' ){
		DT = JSON.parse( input.DT );
	}

	var data = {A:input.A, C:input.C, U:input.U, DT:DT};
...
exports.updateActiveUser = function(input, done) { // A, C, U, DT
	...
	SessionUser.findOne(data,function(err,doc){
		...
		if(doc){
			...
			var preDT;
			if (typeof doc.DT == 'string') {
			  preDT = JSON.parse( doc.DT ); 
			} else {
			  preDT = doc.DT;
			}

			for( var key in input.DT ){
				preDT[key] = input.DT[key];
			}
			doc.DT = preDT;
...
/**
 * User 
 * @name retrieveActiveUserCount
 * @function
 * @param {object} input - JSON 형태의 data
 * @param {callback} done - 조회 후 수행할 callback function
 */
exports.retrieveActiveUserCount = function(input, done) { // A, C
  // 0 : 일반사용자, 2 : 매니저
  var query = { A: input.A, C: input.C, 'DT.roomLevel' : { $in : ['0','2'] } };
  
  SessionUser.count(query, function(err,res){
	if(err) return done(err);
