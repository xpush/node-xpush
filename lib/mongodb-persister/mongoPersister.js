...
exports.searchUser = function (_app, _keys, _values, done){
  ...
  for(var i; i<_keys.length; i++) {
    query[_keys[i]] = _values;
  }
  ...
  User.find(
    query, 
    { userId: 1, deviceType: 1, notiId: 1, users: 1,  _id:0 },
    function(err, doc) {
    ...
    function(callback){
      ...
      if(_userIds){
        ...
        if(isExistedUsers) {
          User.find(
            query, 
            { userId: 1, deviceType: 1, notiId: 1, _id:0 }, // sessionId: 1,   ?
            function(err, doc) {
...
exports.addChannelUser = function (_app, _channel, _userId, done) {
	...
	// TODO 추가 할때 notiId와 deviceId 는?
  Channel.update(
    { 'app':          _app, 
      'channel':      _channel},
    { '$push': { 'users': {'userId': _userId }} },
    { "multi" : true },
    function (err, data){
      if (err) {
        return done(err);
