...
exports.registerUser = function(input, done) { // A, U, PW, DT, D
...
exports.searchUser = function(_A, _keys, _values, done) {
...
exports.createChannel = function(input, done) { // A, C, U
...
exports.addChannelUser = function(input, done) { // A, C, U, DT
  ...
  async.waterfall([
	  ...
	  function(callback) {
		...
		// @ TODO check update query !!!! Why too many ?
...
exports.storeMessages = function(input, done) { // A, C, NM, DT, US, CD
...
exports.unReadMessages = function(input, done) { // A, C, U, D
  ...
  UnreadMessage.find(queryCond, {'_id': 0,'NM': 1,'MG': 1,'CD': 1} ).populate('MG').exec(function(err, messages) {
    if (err) {
      done(err);
    } else {
      done(null, messages);
