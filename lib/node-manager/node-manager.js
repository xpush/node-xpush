  ...
  var connectTry = function(){
    ...
    if(!self.connected){
      if(self.connectionTryNum>3){
        console.log('Zookeeper error encountered [Error: failed to connect]]'); 
        if(callback) callback();
...
NodeManager.prototype._createZnode = function (nodePath, callback) {
  console.log( ' constants.BASE_ZNODE_PATH + nodePath : '+ constants.BASE_ZNODE_PATH + nodePath );
...
NodeManager.prototype._removeZnode = function (nodePath, callback) {
  this.zkClient.remove(
    ...
    function (error) {
      if (error) {
        console.log('Failed to remove node: %s due to: %s.', constants.BASE_ZNODE_PATH+nodePath, error);
