      ...
      function (error, stat) {
        ...
        if (!stat) {
          self.zkClient.create(
            ...
            function (error) {
              if (error) {
                console.log('Failed to create node: %s due to: %s.', self._ROOT_ZNODE_ROOT, error);
              } else {
...
NodeManager.prototype._ROOT_ZNODE_ROOT = '/xpush_servers';

NodeManager.prototype._watchServerNodes = function(){

console.log(this._ROOT_ZNODE_ROOT);
  var self = this;
