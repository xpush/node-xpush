...
var ConsistentHashing = exports.ConsistentHashing = function(nodes, options) {
  ...
  for (var i = 0; i < nodes.length; i++) {
    var c = nodes[i].indexOf('^');
    var n = nodes[i].substr(0, c);
    //var s = nodes[i].substr(c+1);
    //this.addNode(n, s);
...
ConsistentHashing.prototype.addNode = function(name, node) {
...
ConsistentHashing.prototype.removeNode = function(name) {
...
ConsistentHashing.prototype.getNode = function(key) {
  ...
  return {name: result.split('^')[0], url: result.split('^')[1]};
