...
function createSessionServer(options) {
  ...
  var sessionServer = require('./xpush-session-server.js').init(server, options, function(err, result){
...
function createChannelServer(options) {
  ...
  var channelServer = require('./xpush-channel-server.js').init(server, io, options, function(err, result){
    server.listen(port, function () {
