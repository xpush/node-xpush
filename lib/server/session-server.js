...
var SessionServer = exports.SessionServer = function(options, cb) {
  ...
  this.conf.server = {
    MAX_CONNECTION : 50 // max number of sockets per a channel.
  };
    ...
    function(callback) {
      ...
      GCon.onAll(function(key){
        ...
        GCon.getConfig( key, function(value){
          self.conf.server[key.toUpperCase()] = Number(value);
        });
...
SessionServer.prototype.PATH = {
  NODE : '/node/:app/:channel',
  AUTH : '/auth'
};
...
SessionServer.prototype.startup = function() {
  ...
  this.server.listen(this.conf.port, function() {
     ...
     */
    if(!self.methods.GETS.hasOwnProperty(self.PATH.NODE)) {
      self.server.get(self.PATH.NODE, function(req, res, next) {
     ...
     */
    if(!self.methods.POSTS.hasOwnProperty(self.PATH.AUTH)) {
      self.server.post(self.PATH.AUTH, function(req, res, next) {
    ...
    self.emit('started', /*self.server.url // http://0.0.0.0:8000 */
