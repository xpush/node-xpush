...
SessionServer.prototype.startup = function() {
  ...
  this.server.post('/auth', function(req, res, next) {
    ...
    var err = serverUtils.validEmptyParams(req, ['A', 'U', 'D', 'PW']);
    if (err) {
      res.send({
        status: 'error',
        message: err
    ...
    mongoPersister.retrieveUser( {
     ...
     D: req.params.D,
      ...
      var _pw = serverUtils.encrypto(req.params.PW);
      ...
      if (user.PW == _pw) {
        mongoPersister.updateUserToken({
          ...
          U : user.U,
          D : req.params.D,
          TK: serverUtils.randomString(10) // <- Auth Token
        }, function(err, token) {
...
SessionServer.prototype.send = function(params) { // A, C, S, SS, NM, DT
  ...
  if (!_server) {
    ...
    this.getChannelServer(params.A, params.channel, function(_serverInfo) {
