...
var ApiServer = exports.ApiServer = function (options) {
  ...
  this.message_process = require('child_process').fork('../lib/server/message-process.js');
...
ApiServer.prototype.send = function (app, channel, name, data, sessionId) {

console.log('asdf');
  if(this.io.sockets.in(channel) != undefined){
    this.io.sockets.in(channel).emit(name, data);
  }
  ...
  console.log(this.io.sockets.clients(channel));

  // TODO for yohan!!!!
  //this.message_process.send(app, channel, name, data);
  //this.message_process.send(app, channel, name);
