...
ApiServer.prototype.startup = function () {
  ...
  this.io.sockets.on('connection', function (socket) {
    ...
    // params : app, usreId, server
    socket.on('login', function (params, callback) {
      
      socket.sessionId = params.server + '^' + socket.id;

      mongoPersister.updateUserSessionId(params.app, params.userId, socket.sessionId, function (err){
        if(err){
          ...
          if(callback) callback({
            status: 'ok',
            result: {sessionId: socket.sessionId}
          });
        }
      });
    ...
    // params : app, userId
    socket.on('channels', function (params, callback) {
      
      mongoPersister.getChannelsByUserId(params.app, params.userId, function (err, channels){
    ...
    // params : app, userId
    socket.on('createChannel', function (params, callback) {
    ...
    // params : appId, channel, userId
    socket.on('joinChannel', function (params, callback) {

      mongoPersister.joinChannel(params.app, params.channel, params.userId, socket.sessionId, function (err, channels){
        if(err){  console.log(err);
          if(callback) callback({
            status: 'error',
            message: err
