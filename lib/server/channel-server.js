  ...
  this.io.of('/session').use(function(socket, callback) {
    ...
    mongoPersister.retrieveUser(_app, _userId, _deviceId, function(err, user) {
      ...
      if (!user) {
        ...
        if (user.devices[_deviceId].token == _token) {
    ...
    self.socketIds[
    socket.handshake.query.app + '_' + socket.handshake.query.userId + '_' + socket.handshake.query.deviceId] = socket.id;

    console.log('socketIds key = ' + socket.handshake.query.app + '_' + socket.handshake.query.userId + '_' + socket.handshake.query.deviceId);
    ...
    socket.on('channel-create', function(params, callback) {
      ...
      mongoPersister.createChannel({
        ...
        channel: params.channel, // optional (can auto-generate !)
    ...
    socket.on('channel-get', function(params, callback) {
      ...
      mongoPersister.getChannel(socket.handshake.query.app, params.channel, function(err, channel, msg) {
        if (err) {
          console.log(err);
          if (callback) callback({
            status: 'error',
    ...
    socket.on('group-add', function(params, callback) {
      var err = serverUtils.validSocketParams(params, ['userIds', 'groupId']);
      if (err) {
        callback(err);
        return;
      }
...
*/
    ...
    socket.on('disconnect', function() {
      ...
      socket.handshake.query.app + '_' + socket.handshake.query.userId + '_' + socket.handshake.query.deviceId];
  ...
  this.io.of('/channel').use(function(socket, callback) {
      ...
      function(err, datas) {
        ...
        for (var x = 0; x < datas.length; x++) {
          self.channels[socket.handshake.query.app + '^' + socket.handshake.query.channel].push({
            userId: datas[x].userId,
            deviceId: datas[x].deviceId,
            notiId: datas[x].notiId
          });
        }
    ...
    ss(socket).on('file-upload', function(stream, data, callback) {
  ...
  self.sessionSubscriber.on('message', function(receivedData) {
    ...
    if (receivedData._type == 'message') {
      ...
      receivedData.app + '_' + receivedData.userId + '_' + receivedData.deviceId];
...
ChannelServer.prototype.send = function(_app, _channel, _name, _data, callback) {
  ...
  if (this.io.of('/channel'). in (_room) != undefined) {
    ...
    _data.timestamp = currentTimestamp;
...
*/
  ...
  for (var x = 0; x < users.length; x++) {
    ...
    if (!_tmpIds[users[x].userId + "^" + users[x].deviceId]) {
      ...
      console.log('Message to SessionServer - ' + users[x].userId + " - " + users[x].deviceId);
      self.sessionManager.publish(
      serverNode.name, {
        _type: 'message',
        /* IMPORTANT */
