...
ChannelServer.prototype.startup = function () {
  ...
  this.io.of('/session').authorization(function (handshakeData, callback) {
    console.log("고은산:/session authorization");
...
*/
  ...
  this.io.of('/channel').authorization(function (handshakeData, callback) {
    ...
    if(_mode == 'CHANNEL_ONLY'){ // without session socket Server.
      console.log("channelOnly");
        self.channels[_app+'^'+_channel] = [
          {
            userId: _userId,
            deviceId: _deviceId
          }//,
	  //{ userId:'james@xpush.com',
	  //  deviceId:'35424905084289',
		//notiId:'APA91bF2tNr7-I14LtfU-o1dRsadt0NQzD2dFDMiN9uLiekSL3NodsoYDpMCwYtoQ-7fysasga-GbdxhnZwrassi3OPXOiC9zsm11Ol__h3fCYQ1x_M1zohBE5f7psDO3XobjQJTShs8YBT0R-VOCXFUceoYIVSPvQ'
	  //},
    //{ userId:'esko@esko.com',
    //  deviceId:'web'
    //}
        ];
        callback(null, true);
    ...
    socket.on('join', function (user, callback) {
      // userId 만 넘어오면 찾아서 넣도록 해야 하지 않을까? @ TODO
      self.channels[socket.handshake._app+'^'+socket.handshake._channel].push(user);
      if(callback) callback({
        status: 'ok'
      });
    });
  ...
  self.sessionSubscriber.on('message', function(receivedData) {
    ...
    if(receivedData._type == 'message'){
      ...
      console.log(receivedData.app+'_'+
        receivedData.userId+'_'+
        receivedData.deviceId);
      console.log(self.socketIds);
...
ChannelServer.prototype.send = function (_app, _channel, _name, _data, callback) {
  ...
  if(this.io.of('/channel').in(_room) != undefined){
    ...
    console.log(self.io.of('/channel').clients(_room).blue);
  }
