...
ChannelServer.prototype.startup = function() {
  ...
  //file upload by multipart HTTP POST
  ...
  this.io.of('/session').use(function(socket, callback) {
    ...
    socket.on('channel-update', function(callback) {
    ...
    socket.on('user-query', function(params, callback) {
      ...
      if(params.query){
        for(var key in params.query){
          var attrName = key;
          var attrValue = params.query[key];

          if(typeof attrValue == 'string' || attrValue instanceof String) {
            if(attrValue.indexOf('%') == 0 && (attrValue.lastIndexOf('%')+1) == attrValue.length){
              params.query[attrName] = new RegExp( attrValue.substring(1, attrValue.lastIndexOf('%')), 'i');
            }
          }
        }
      }
  ...
  self.sessionSubscriber.on('message', function(receivedData) {
    console.log("=============== session message",receivedData);
