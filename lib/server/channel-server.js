...
ChannelServer.prototype.startup = function() {
  ...
  this.io.of('/channel').use(function(socket, callback) {
    ...
    mongoPersister.addActiveUser({
      ...
      console.log("====save user session to mongod", arguments);
      if(err){
        if (callback) callback({ status: 'ERR-INTERNAL', message: err });
      }
    });
        ...
        if (err) {
          ...
          if(data){
            ...
            mongoPersister.retrieveActiveUser({
              ...
              if(doc){
                for(var i=0;i<doc.US.length;i++){
                  var au = doc.US[i];
                  var nm = Object.keys(au);
                  self.sessionManager.updateClient(
                    socket.handshake.query.A,
                    socket.handshake.query.C,
                    nm,
          ...
          if (err) {
            ...
            if(data&&callback){
              ...
              mongoPersister.retrieveActiveUser({
                ...
                if(doc){
                  for(var i=1;i<doc.US.length;i++){
                    _.extend(doc.US[0],doc.US[i]);
                    
                    if(i==doc.US.length-1){
                      console.log(doc.US[0]);
  ...
  this.io.of('/admin').use(function(socket, callback) {
    ...
    socket.on('usage', function(callback) {
      ...
      result.data = {
        socket : Object.keys(self.socketIds).length,
  ...
  self.sessionSubscriber.on('message', function(receivedData) {
    ...
    if (receivedData._type == 'message') {
    ...
    } else if (receivedData._type == 'send-online') {

      self.sendOnline(receivedData.A, receivedData.C, receivedData.NM, receivedData.DT);
      ...
      if(self.channels[receivedData.A + '^' + receivedData.C]){
        ...
        if(_mc){
          ...
          if(_mc.indexOf(receivedData.S)==-1) _mc.push(receivedData.S);
        }
      }
      ...
      if(_mcd){
        _mcd.splice(_mcd.indexOf(receivedData.S), 1);
...
ChannelServer.prototype.sendOnline = function(_app, _channel, _name, _data, callback) {
