...
ChannelServer.prototype.startup = function() {
  ...
  var serverOption = {};
  if( self.options.protocol == 'https' && self.options.ssl ){
    var key = fs.readFileSync( self.options.ssl.key );
    var cert = fs.readFileSync( self.options.ssl.cert );

    serverOption.key = key;
    serverOption.cert = cert;
  }else{
  ...
  console.log(self.options.protocol);
  var http = require( self.options.protocol ).Server(app, serverOption);
  ...
  this.io.of('/channel').use(self._channel_authorization).on('connection', function(socket) {
    ...
    mongoPersister.addActiveUser({
      ...
      if(err){
        console.log(err);
        socket.emit( "connect_error", { status: 'ERR-INTERNAL', message: err } );
        socket.disconnect();
        return;
    ...
    for (var key in self.methods.CHANNEL_SOCKET) {
      if(key != 'connection') socket.on(key, self.methods.CHANNEL_SOCKET[key]);
    }
  ...
  http.listen(this.options.port, function() {
    self.emit('started', self.options.protocol+"://"+self.options.host, self.options.port);
  });
