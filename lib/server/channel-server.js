...
ChannelServer.prototype.startup = function() {
  ...
  this._channel_send = function(params, callback) {
  ...
  this.io.of('/channel').use(self._channel_authorization).on('connection', function(socket) {
     ...
     * @param {callback} callback - 결과를 callback으로 전송한다.
     */
    socket.on('send', self._channel_send);
  ...
  self.sessionSubscriber.on('_message', function(receivedData) {
    ...
    if (receivedData._type == 'message') {
      ...
      if (receivedData.SS) {
        self.sendPrivate(null, receivedData.SS, receivedData.NM, receivedData.DT);
      } else {
      ...
      if (receivedData.SS) {
        ...
        self.sendOnce(receivedData.A, receivedData.C, receivedData.NM, receivedData.DT);
      }
 ...
 */
ChannelServer.prototype.sendPrivate = function(_server, _socketId, _name, _data, callback) {
...
ChannelServer.prototype.sendOnce = function(_app, _channel, _name, _data, callback) {
...
ChannelServer.prototype.ping = function() {
  console.log(this.proc);
  return 'pong';
};

ChannelServer.prototype.session = {
  ping: function(){
    return this.ping();
  },
  send: function(params, callback) {
    return this._channel_send(params, callback);
  }
};
