...
exports.likeQueryMaker = function( data ){
  ...
  for( var k in data ){
    ...
    if(typeof inValue == 'string' || inValue instanceof String) {
      if(inValue.indexOf('%') == 0 && (inValue.lastIndexOf('%')+1) == inValue.length){
        result[k] = new RegExp( inValue.substring(1, inValue.lastIndexOf('%')), 'i');
      } else {
        result[k] = inValue;
      }      
      ...
      if( Array.isArray( inValue ) ){
        var newArray = [];
        for( var inx in inValue ) {
          newArray.push( this.likeQueryMaker( inValue[inx] ) );
        }
        result[k] = newArray;
      } else {
        result[k] = this.likeQueryMaker( inValue );
      }      
    }
  }
