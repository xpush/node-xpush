...
ChannelServer.prototype._startup = function (server, io) {
  ...
  io.of('/channel').use(function (socket, next) {
      ...
      // Over
      if( !self.isNodeChanging && _count_of_this_channel >= ( LIMIT_COUNT + BUFFER_COUNT ) && MIN_REPLICAS != self.replicas ){
        self._changeReplicas( MIN_REPLICAS ); 
      }
...
ChannelServer.prototype._channel_disconnect = function(_this) {
  ...
  // Under
  if( !self.isNodeChanging && _count_of_this_channel <= ( LIMIT_COUNT - BUFFER_COUNT ) && self.startReplicas != self.replicas ){
