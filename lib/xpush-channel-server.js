...
ChannelServer.prototype._startup = function (server, io) {
  ...
  io.of('/channel').use(function (socket, next) {
      ...
      var connectionCount = Object.keys(self.io.of('/channel').connected).length;
      ...
      socket.on('send', function (params, callback) {
        ...
        if (self.methods.CHANNEL_SOCKET.hasOwnProperty('send')) self.methods.CHANNEL_SOCKET.send(socket);
...
ChannelServer.prototype._channel_disconnect = function (_this) {
  ...
  if (_count_of_this_channel == 0) {
    ...
    if(_socket['__sessionData']) {
      self.sessionManager.deleteClient( _a, _c, _u);
    }
  }
...
ChannelServer.prototype.getSessionCount = function (_socket, _fn) {

  this.sessionManager.retrieveClientsCount(
    _socket.handshake.query.A,
    _socket.handshake.query.C,
    function (err, data) {
      if (err) {
        if (_fn) _fn({status: 'ERR-INTERNAL', message: err});
        return;
      } else {
        if (data !== undefined && _fn) {
          _fn({
            status: 'ok',
            result: data
          });
...
ChannelServer.prototype.getSessionList = function (_socket, _fn) {

  this.sessionManager.retrieveClients(
    _socket.handshake.query.A,
    _socket.handshake.query.C,
    function (err, data) {
      if (err) {
...
ChannelServer.prototype.setSessionData = function (_socket, _fn) {

  this.sessionManager.updateClient(
    _socket.handshake.query.A,
    _socket.handshake.query.C,
    _socket.handshake.query.U,
    _socket.handshake.query.DT);

  // TODO 비동기 처리 확인 필요!
  _socket['__sessionData'] = true;

};

ChannelServer.prototype.getSessionData = function (_socket, _fn) {
