          ...
          self.nodeManager = new NodeManager(
            ...
            function(err, message) {
              if (!err) {
                ...
                self.nodeManager.addServerNode(self.conf, startReplicas, function(err, path, replicas) {
...
ChannelServer.prototype._startup = function(server, io) {
  ...
  this.io.of(NAMESPACE).use(function(socket, next) {
    ...
    self.calculateReplicas( NAMESPACE, 'CONNECTION' );
    ...
    if (self.methods.CHANNEL_SOCKET.hasOwnProperty('connection')) self.methods.CHANNEL_SOCKET.connection(_msgObj, socket);

    // 동일한 socket을 사용 중인 user에게 `CONNECTION` EVENT를 발생시킨다.
...
ChannelServer.prototype.calculateReplicas = function(nsp, event){

  var self = this;

  if( event == 'CONNECTION'){

    var connectionCount = Object.keys(self.io.of(nsp).connected).length;
...
ChannelServer.prototype._channel_disconnect = function(_this) {
  ...
  self.calculateReplicas( NAMESPACE, 'DISCONNECT' );
...
ChannelServer.prototype.send = function(argJson) {
  ...
  if (argJson.socketId) {
    this._sendPrivate(argJson.server, argJson.socketId, argJson.name, argJson.data, argJson.namespace);
  } else {
    this._sendOnce(argJson.app, argJson.channel, argJson.name, argJson.data, argJson.namespace);
  }
