var io = require('socket.io-client'), util = require('./util'), faker = require('faker');
...
var run = function () {
  ...
  util.get(_host, _port, '/node/' + app + '/' + channel, function (err, data) {
    ...
    var query = 'A=' + app + '&' + 'C=' + channel + '&' + 'U=' + uid + '&' + 'D=DEVAPP&' + 'S=' + data.result.server.name + '&' + 'DT=' + JSON.stringify(DT) + '&' + 'MD=CHANNEL_ONLY';
    ;
