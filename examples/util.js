var http = require('http'), fs = require('fs'), path = require('path'), mime = require('mime');
exports.post = function (host, port, path, data, cb) {
  var dataObject = JSON.stringify(data);
  var postheaders = {'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(dataObject, 'utf8')};
  var optionspost = {host: host, port: port, path: path, method: 'POST', headers: postheaders};
...
exports.postFile = function (host, port, path, data, cb) {
  var options = {host: host, port: port, path: path, method: 'POST'};
  options.headers = {
    'XP-A': data.appId,
    'XP-C': data.channel,
    'XP-U': JSON.stringify({U: data.userId, D: data.deviceId})
  };
...
exports.get = function (host, port, path, cb) {
  var optionsget = {host: host, port: port, path: path, method: 'GET'};
